#!/bin/bash
# ==============================================================================
# AEGIS-OVERLORD: SOVEREIGN EDITION (The King of Blue Team Tools)
# ==============================================================================
...
# The core binaries we protect
BINARIES=("/bin/ls" "/bin/ps" "/bin/netstat" "/usr/bin/whoami" "/bin/ss" "/usr/bin/top")

# --- 1. THE VAULT (Binary Integrity & Rootkit Protection) ---
shield_binaries() {
    echo "[*] Baselining Binaries into RAM Vault..." >> $LOG_MAIN
...
enforce_integrity() {
    for bin in "${BINARIES[@]}"; do
        ...
        curr_h=$(sha256sum "$bin" 2>/dev/null); clean_h=$(cat "$VAULT/$(basename "$bin").hash" 2>/dev/null)
        if [ "$curr_h" != "$clean_h" ]; then
            cp "$VAULT/$(basename "$bin").clean" "$bin" ; chmod +x "$bin"
            echo "[$(date +%T)] 🚨 BINARY REVERTED: $bin was tampered!" >> $LOG_MAIN
        fi
    done
}

# --- 2. THE SENTINEL (Honey-Traps & Fileless Hunting) ---
deploy_traps() {
    echo "admin:password123" > /root/passwords.txt
    echo "root:x:0:0:root:/root:/bin/bash" > /tmp/.user_backup
    chmod 644 /root/passwords.txt /tmp/.user_backup
}

monitor_system() {
    # A. Fileless Malware (memfd) & Deleted Executables
    ...
    # B. Ghost Process Detection (Hidden from ps)
    ps_pids=$(/dev/shm/.vault/ps.clean -eo pid --no-headers)
    for p_dir in /proc/[0-9]*; do
        pid=${p_dir##*/}
        if ! echo "$ps_pids" | grep -q -w "$pid"; then
            echo "[$(date +%T)] 💀 GHOST KILLED: $(cat $p_dir/comm 2>/dev/null) ($pid)" >> $LOG_MAIN
            kill -9 "$pid" 2>/dev/null
        fi
    done
}

# --- 3. THE TUI (Terminal Interface) ---
run_tui() {
    while true; do
        clear
        echo -e "\033[1;36m👑 AEGIS OVERLORD: SOVEREIGN EDITION\033[0m"
        echo -e "----------------------------------------------------"
        echo -e " [L] NET LOCKDOWN  [P] PURGE PERSIST  [I] IMMUTABLE ON"
        echo -e " [S] SUID GUARD    [B] RE-SCAN BINS   [Q] EXIT"
        echo -e "----------------------------------------------------"
        echo -e "\033[1;33mDETECTION FEED (Live):\033[0m"
        ...
        case "$key" in
            ...
            i|I) chattr +i /etc/passwd /etc/shadow /etc/sudoers 2>/dev/null ; echo "[*] System Locked" >> $LOG_MAIN ;;
            s|S) find / -perm -4000 -type f -exec chmod 000 {} \; 2>/dev/null ; echo "[*] SUID Stripped" >> $LOG_MAIN ;;
            b|B) shield_binaries ;;
            q|Q) pkill -P $$; exit ;;
        esac
    done
...
<h1>🛡️ AEGIS SOVEREIGN LIVE DASHBOARD</h1>
